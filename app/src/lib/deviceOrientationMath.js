...
export function getPointingVector(alpha, beta, gamma) {
  ...
  // Negated: `alpha` increases counter-clockwise (standard math convention) while compass
  // azimuth increases clockwise (N→E→S→W) — without the flip, every direction except due
  // north/south came out mirrored (e.g. facing east read as facing west), which is exactly the
  // kind of bug that leaves a target near north looking plausible while anything else is
  // completely wrong. Verified numerically: alpha 0/45/90/.../315° now maps to azimuth
  // 0/45/90/.../315° instead of 0/315/270/.../45°, with elevation unaffected either way.
