 ...
 */
export function getPointingDirection(alpha, beta, gamma) {
  ...
  const azimuthDeg = ((Math.atan2(worldX, worldY) * 180) / Math.PI + 360) % 360
  const elevationDeg = (Math.asin(Math.max(-1, Math.min(1, worldZ))) * 180) / Math.PI
