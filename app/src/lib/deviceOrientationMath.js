...
export function getPointingVector(alpha, beta, gamma) {
  ...
  // NOT negated. `alpha` maps to azimuth as (360 - alpha) mod 360, not azimuth = alpha — that's
  // not a bug, it's the actual, correctly-derived relationship for this rotation order and
  // world-frame convention, cross-checked term-by-term against the reference rotation matrix
  // from the W3C DeviceOrientation spec's own implementation appendix (the "cY*sZ*sX + cZ*sY"
  // form used by e.g. full-tilt.js) across a full grid of alpha/beta/gamma combinations, with
  // zero divergence outside the inherently ambiguous exactly-vertical case. A previous attempt
  // to "fix" this by negating alpha was itself the bug — it broke a formula that was already
  // correct, on the strength of a flawed intuition (that azimuth should equal alpha directly).
