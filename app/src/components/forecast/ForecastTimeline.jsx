...
export default function ForecastTimeline({ location, weather, target }) {
  const [selectedNight, setSelectedNight] = useState(null)

  const nights = useMemo(
    () => computeForecast(weather, location.latitude, location.longitude, 15, location.population, target),
  ...
  return (
    <div className="card">
      ...
      {selectedNight && <ForecastDayModal night={selectedNight} onClose={() => setSelectedNight(null)} target={target} />}
    </div>
  )
}
