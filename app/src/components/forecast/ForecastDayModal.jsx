...
import { formatDayLabel, formatPercent, formatTime, formatWindKmh, formatDirectionPhrase } from '../../utils/format'
import { BORTLE_LABELS } from '../../lib/bortle'
import { IconClose, IconTelescope } from '../icons/Icons.jsx'
import PlanetIcon from '../events/PlanetIcon.jsx'
...
export default function ForecastDayModal({ night, onClose, target = 'stars' }) {
  return createPortal(
    <div className="fixed inset-0 z-40 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      ...
      <div className="relative w-full max-w-md bg-night-surface border border-night-border rounded-2xl p-5 shadow-lg shadow-black/20 animate-fade-in-up">
        ...
        <div className="flex items-center gap-4 mb-5">
          ...
          <div>
            ...
            <p className="text-xs text-muted">
              {target === 'moon' ? 'Ce soir/cette nuit du ' : 'Nuit du '}
              {formatTime(night.nightStart)} au {formatTime(night.nightEnd)}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 text-sm mb-4">
          ...
          <Row label="Risque de pluie" value={formatPercent(night.maxRainProbability)} />
        ...
        {target === 'planets' && (
          <div className="mb-4">
            <p className="text-xs text-muted uppercase tracking-wide mb-2">Planètes visibles cette nuit</p>
            {night.visiblePlanets?.length ? (
              <div className="flex flex-col gap-2">
                {night.visiblePlanets.map((planet) => (
                  <div key={planet.name} className="flex items-center justify-between gap-2 text-sm bg-night-bg border border-night-border rounded-lg px-3 py-2">
                    <span className="flex items-center gap-2">
                      <PlanetIcon name={planet.name} size={18} />
                      <span className="font-medium">{planet.frenchName}</span>
                    </span>
                    <span className="text-brand-light text-xs">
                      {formatTime(planet.visibleFrom)}–{formatTime(planet.visibleUntil)} · {formatDirectionPhrase(planet.direction)}
                    </span>
                  </div>
                ))}
              </div>
              ...
              <p className="text-sm text-muted bg-night-bg border border-night-border rounded-lg px-3 py-2">
                Aucune planète visible cette nuit-là.
              </p>
            )}
          </div>
        )}
        ...
        {night.clearWindow ? (
          <p className="flex items-center gap-2 text-sm bg-night-bg border border-night-border rounded-lg px-3 py-2">
            ...
            {target === 'moon' ? 'Meilleure fenêtre pour la lune' : 'Meilleure fenêtre'} :{' '}
            <span className="text-brand-light font-medium">{formatTime(night.clearWindow.start)}–{formatTime(night.clearWindow.end)}</span>
          </p>
        ) : (
          <p className="text-sm text-muted bg-night-bg border border-night-border rounded-lg px-3 py-2">
            {target === 'moon'
              ? 'Lune non visible ce soir/cette nuit, ou pas de créneau dégagé.'
