...
export default function SkyCompass({ label, azimuthDeg, altitudeDeg, onClose }) {
  ...
  // Near the zenith, azimuth stops meaning much — a target 85° up is basically "overhead" no
  // matter which way you're facing — so widen the tolerance smoothly as the target approaches
  // straight up, instead of demanding a heading match that becomes physically noisy and largely
  // meaningless there.
  const azimuthTolerance =
    altitudeDeg != null ? AZIMUTH_TOLERANCE_DEG + Math.max(0, altitudeDeg - 60) * 3 : AZIMUTH_TOLERANCE_DEG

  const belowHorizon = altitudeDeg != null && altitudeDeg <= 0
  const azimuthAligned = heading != null && Math.abs(angleDiff(heading, azimuthDeg)) <= azimuthTolerance
