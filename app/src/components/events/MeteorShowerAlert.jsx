    ...
    <div>
      ...
      <div className="flex flex-col gap-3">
        {upcoming.slice(0, 3).map((shower) => {
          ...
          return (
            <div key={`${shower.name}-${shower.peakDate.getFullYear()}`} className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="flex items-center gap-1.5 font-medium text-sm">
                  <IconComet size={14} className="text-brand-light flex-shrink-0" />
                  {shower.name}
                </p>
                {radiantPosition ? (
                  <button
                    onClick={() => setAiming({ shower, position: radiantPosition })}
                    className="flex items-start gap-1.5 text-xs text-muted hover:text-brand-light transition-colors text-left"
                  >
                    <IconCompass size={12} className="flex-shrink-0 mt-0.5" />
                    <span>
