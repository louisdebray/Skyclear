...
// How fast the iOS magnetic-north calibration offset (see below) is allowed to drift, per sample.
// Deliberately slow: it should track slow real calibration drift, not bounce around with noise.
const CALIBRATION_SMOOTHING = 0.02
 ...
 *
 * Heading always comes from our own rotation-derived azimuth (internally consistent and
 * singularity-free at any tilt), rather than switching to iOS's `webkitCompassHeading` outright:
 * that value is magnetically calibrated but defined by the OS from the device's *top edge*, a
 * definition that only loosely matches "back camera azimuth" once the phone tilts far from
 * vertical-ish, and has its own instability pointing near-straight-up. Instead, when it's
 * available and the device isn't near the zenith (where any heading estimate gets noisy), we use
 * it only to slowly calibrate a magnetic-declination-style offset on top of our own azimuth —
 * best of both: geometry that's always self-consistent, nudged toward magnetic-true over time.
 *
...
export function useDeviceHeading() {
  ...
  const calibrationOffset = useRef(0) // slowly-tracked (webkitCompassHeading - our azimuth), degrees
  ...
  useEffect(() => {
    ...
    function handleOrientation(event) {
      ...
      if (horizontalMagnitude >= MIN_HORIZONTAL_MAGNITUDE || smoothedHeading.current == null) {
        if (typeof event.webkitCompassHeading === 'number' && horizontalMagnitude >= MIN_HORIZONTAL_MAGNITUDE) {
          const offsetSample = angleDelta(azimuthDeg, event.webkitCompassHeading)
          calibrationOffset.current += angleDelta(calibrationOffset.current, offsetSample) * CALIBRATION_SMOOTHING
        }
        const rawHeading = (azimuthDeg + calibrationOffset.current + 360) % 360
